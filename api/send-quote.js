...
export default async function handler(req, res) {
  ...
  try {
    ...
    const smtpUser = (process.env.SMTP_USER || 'transylview3d@gmail.com').trim();
    const smtpPass = (process.env.SMTP_PASS || 'uzfqetathiidwxai').replace(/\s+/g, '');

    // Configure Google Transporter with built-in Gmail service optimization for serverless
    ...
    const ownerMailOptions = {
      from: `"TransylView 3D Leads" <${smtpUser}>`,
    ...
    return res.status(200).json({ success: true, message: 'Solicitarea a fost trimisă cu succes!' });
  } catch (error) {
    console.error('Serverless mail error:', error);
    return res.status(500).json({ error: 'A apărut o eroare la trimiterea e-mailului. Vă rugăm să ne apelați la 0751 801 025.' });
  }
}
